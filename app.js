...
import initOCCT, { readStepFile } from "https://cdn.jsdelivr.net/npm/occt-import-js/dist/occt-import-js.esm.js";
...
let renderer, scene, camera, controls, occt, currentModel, exploded=false;
...
async function init(){
    ...
    occt = await initOCCT({
        locateFile: (file) => `https://cdn.jsdelivr.net/npm/occt-import-js/dist/${file}`
    });
...
async function loadStepFromBuffer(buf,name){
    if(!occt){ setStatus("Импортёр не инициализирован."); return; }
    try{
